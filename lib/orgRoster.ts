...
const MANAGED_ROLES = ["volunteer", "org_admin"] as const;

// The `where` for the active-members roster. Partners are always included;
// managed roles (volunteer/org_admin) are included only when scoped to
// `adminOrgId`. A null org (an admin not linked to one) shows partners only —
